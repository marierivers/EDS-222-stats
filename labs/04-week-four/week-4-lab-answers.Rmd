...
rootdir <- ("~/Documents/UCSB_Environmental_Data_Science/EDS_222_Statistics_for_Environmental_Data_Science/labs/EDS-222-stats")
...
```{r}
lm(hwy ~ year, data = mpg) %>% # R controls which is the reference variable (unless you tell it)
  summary() %>%
  xtable() %>%
  kable()
```
...
the slope is change per category (ie difference between vintage 1999 cars and vintage 2008 cars), not year. The slope, beta_1 is the difference between the mean in 1999 and the mean in 2008
...
-   **Coefficient on year:** Highway fuel efficiency is, on average, 1.40 miles per gallon higher in cars built in 2008, as compared to 1999, holding fixed the engine size of the car. This coefficient is larger than in the simple linear regression, suggesting that engine size is correlated with both fuel efficiency and time. Here, it appears that in later years engine sizes were *larger*, which lowers fuel efficiency. Once we control for that effect, the role of time itself on the efficiency is much more substantial.
...
augment(mod) # this augment command generates columns of useful things such as
# gives a fitted value and residual for every observation
# the ".fitted" column is the predicted values based on the model
```
...
mpg %>% 
  ggplot(aes(x = displ, y = hwy, color = year)) +
  geom_point() +
  geom_line(data = augment(mod), aes(y = .fitted, color = year)) + # remains the x values from 2 lines of code above
  labs(x = "Engine Displacement, in litres",
       y = "Highway Miles per gallon") +
