...
# inputs for lm function: data set, independent variable, dependent variable (double check order of these)
lm(total_l ~ tail_l, data = possum) %>% 
  tidy %>% 
  gt()

# total length is a function of body length + tail length
...
lm(head_l ~ tail_l, data = possum) %>% 
  tidy %>% 
  gt()
# a problem with beta knot is that it mathematically, it results in body length for a possum with a tail length of 0. NOTE: don't extrapolate beyond your data.
```
...
ggplot(data = possum, aes(y = total_l, x = tail_l)) +
  ...
  labs(x = "Length of Possum Tail (cm)",
       y = "Total Length of Possum (cm)")
# put your regression into ggplot, data, y axis (dependent variable), x-axis (independent variable), relate y to x, se=false <- don't give me standard errors
```
