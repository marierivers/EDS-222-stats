...
# rootdir <- ("~/Documents/UCSB_Environmental_Data_Science/EDS_222_Statistics_for_Environmental_Data_Science")
...
p_proportion <- loans %>%
  count(homeownership) %>%
  mutate(proportion = n / sum(n)) %>% # the n comes from the count step
...
p_stacked <- ggplot(loans, aes(x = homeownership, fill = application_type)) + # note use of a variable for fill color
  geom_bar() +
